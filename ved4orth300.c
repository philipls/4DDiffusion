...
%  [T, Gt, Ptheta, Mt] = vd4orth300([1.0,1.0,0,0,1.0, 0.9], 1.5, 5);
%  
...
#define NP 10     /* Number of input parameters */
...
void ved4orth300(double *T, double *Gt, double *Ft, double *Ptheta, double *Mt, double *P, double tmax, int badix) {
/* -----------------------------------------------------------------------------------------------
...
------------------------------------------------------------------------------------------------ */
    ...
    /* Pool within orthants - first half <= pi/2, second half greater than pi/2 */
    for (i = 0; i < npsi; i++) {
        if (i < npsi / 2) {
             orth1 = 0;
        } else {
        ...
        for (j = 0; j < nphi; j++) {
            ...
            for (k = 0; k < ntheta; k++) {
                ...
                ijk = (nphi * npsi) * k + npsi * j + i;
                Ptheta[o] += Pijk[ijk];
                Mt[o] += Mijk[ijk] * Pijk[ijk];
                for (l = 0; l < sz; l++) { 
                    ijkl = (ntheta * nphi * npsi) * l + (nphi * npsi) * k + npsi * j + i;
                    Gt[north * l + o] += Gtijk[ijkl];
                }
            }
        }
    }
    for (o = 0; o < north; o++) {
        Mt[o] = Mt[o] / Ptheta[o];
        for (l = 1; l < sz; l++) {
            Ft[north * l + o] += Ft[north * (l - 1) + o] + (Gt[north * l + o] + Gt[north * (l - 1) + o]) * h / 2.0;
        }
 ...
 */
    ...
    if (nrhs != 3) {
         mexErrMsgTxt("vd4orth300: Requires 3 input args.");
    } else if (nlhs != 5) {
        mexErrMsgTxt("vd4orth300: Requires 5 output args."); 
    }
   ...
   /* Mt */
    ...
    /*
      ...
      -----------------------------------------------------------------------
    */
    ved4orth300(T, Gt, Ft, Ptheta, Mt, P, tmax, badix); 
}
